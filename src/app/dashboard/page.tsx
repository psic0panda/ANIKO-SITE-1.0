    ...
    <main className="min-h-screen bg-slate-50 text-brand-primary pb-20">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 md:px-12 grid gap-12 lg:grid-cols-[1fr_350px]">
        ...
        <div className="space-y-12">
          ...
          {videos.length > 0 && (
            <section className="bg-white rounded-[3rem] p-8 md:p-10 shadow-xl border border-white animate-fade-up">
              <div className="flex items-center gap-4 mb-8">
                <div className="h-12 w-12 rounded-2xl bg-brand-primary/10 flex items-center justify-center text-2xl">📈</div>
                <div>
                  <h3 className="text-2xl font-black">Jornada do {profile.child_name?.split(' ')[0]}</h3>
                  <p className="text-slate-500 text-sm font-medium">Acompanhe a evolução e os temas trabalhados.</p>
                </div>
              </div>
              ...
              <div className="relative pt-8 pb-4">
                ...
                <div className="relative flex justify-between items-center gap-4 overflow-x-auto pb-6 px-4 no-scrollbar">
                  {[...videos].reverse().map((v, idx) => {
                    const categoryMap: any = {
                      'Higiene': { icon: '🛁', color: 'bg-blue-500' },
                      'Social': { icon: '🤝', color: 'bg-green-500' },
                      'Aprendizado': { icon: '📚', color: 'bg-purple-500' },
                      'Rotina': { icon: '⏰', color: 'bg-amber-500' },
                      'Comunicação': { icon: '💬', color: 'bg-cyan-500' }
                    };
                    const cat = categoryMap[v.category] || { icon: '✨', color: 'bg-brand-accent' };
                    
                    return (
                      <div key={idx} className="flex flex-col items-center min-w-[140px] group">
                        <div className={`relative h-14 w-14 rounded-2xl ${cat.color} flex items-center justify-center text-2xl shadow-lg z-10 group-hover:scale-110 transition-transform cursor-help border-4 border-white`}>
                          {cat.icon}
                          <div className="absolute -top-1 -right-1 h-4 w-4 bg-white rounded-full flex items-center justify-center shadow-sm">
                            <Star className="h-2 w-2 fill-brand-accent text-brand-accent" />
                          </div>
                        </div>
                        <div className="mt-4 text-center">
                          ...
                          <p className="text-xs font-bold text-brand-primary line-clamp-2 max-w-[120px]">
                            {v.category || 'Especial'}
                          </p>
                        </div>
                        
                        {/* Tooltip de Hover */}
                        <div className="absolute top-[100%] mt-2 opacity-0 group-hover:opacity-100 transition-opacity bg-brand-primary text-white text-[10px] py-2 px-3 rounded-lg whitespace-nowrap z-20 pointer-events-none shadow-xl">
                          {v.title}
                        </div>
                      </div>
                    );
              ...
              <div className="mt-8 pt-8 border-t border-slate-50 flex flex-wrap gap-4">
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Higiene</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full">
                  <span className="h-2 w-2 rounded-full bg-green-500" />
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Social</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full">
                  <span className="h-2 w-2 rounded-full bg-purple-500" />
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Aprendizado</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full">
                  <span className="h-2 w-2 rounded-full bg-brand-accent" />
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Outros</span>
                </div>
              </div>
            </section>
          )}
